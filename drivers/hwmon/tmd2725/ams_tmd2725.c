...
static void tmd2725_set_defaults(struct tmd2725_chip *chip)
{
	...
	sh[TMD2725_REG_PGCFG0] =
	    (chip->params.prox_pulse_cnt - 1) | chip->params.prox_pulse_len;
...
static ssize_t tmd2725_regs_get(struct tmd2725_chip *chip, char *buf,
				int bufsiz)
{
	u8 regval[16];
	int i, j, cnt;
	...
	for (; i < MAX_REGS; i += 16) {
		...
		for (j = 0; j < 16; j++) {
			...
			if (reginuse[(i >> 3) + (j >> 3)] & (1 << (j & 7))) {
				cnt +=
				    snprintf(buf + cnt, bufsiz - cnt, " %02x",
					     regval[j]);
			} else {
...
static ssize_t tmd2725_regs_dump(struct device *dev,
			       ...
			       const char *buf, size_t size)
{
	...
	if (pmask == -1) {
		...
		rc = ams_i2c_modify(chip, chip->shadow, preg, pmask, pval);
	}
...
static int tmd2725_get_id(struct tmd2725_chip *chip, u8 * id, u8 * rev,
			  u8 * auxid)
{
	int ret = 0;
...
* device probe
********************************************************************/
static int tmd2725_probe(struct i2c_client *client,
			 const struct i2c_device_id *idp)
{
	...
	wakeup_source_init(&chip->ws, "tmd2725");
	chip->client = client;
	chip->pdata = pdata;
...
MODULE_DESCRIPTION
    ("AMS-TAOS tmd2725 ALS, Prox, Gesture, Beam, RemCon sensor driver");
MODULE_LICENSE("GPL");
